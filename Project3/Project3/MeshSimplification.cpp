	...
	for (set<int>::iterator it1 = v->connectV.begin(); it1 != v->connectV.end(); it1++)
		for (set<int>::iterator it2 = v->connectV.begin(); it2 != v->connectV.end(); it2++)
			...
			if ((*it1) < (*it2) && (vertex[(*it1)].connectV.count(*it2) > 0)) //
			{
				...
				for (int i = 0; i < 4; i++)
					for (int j = 0; j < 4; j++)
						Q.mat[i][j] += coefficient[i] * coefficient[j];
			}
	return Q;
}
void calculateBest(Pair& pair)
{
	...
	Matrix4 uQ = calQ(u->id);
	Matrix4 vQ = calQ(v->id);
	double Q[4][4];
	double QQ[4][4];//两个误差矩阵之和
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < 4; j++)
		{
			Q[i][j] = QQ[i][j] = uQ.mat[i][j] + vQ.mat[i][j];
		}
...
{cout << "---------------------simplify" << endl;
	for (int i = 0; i < cntDelFace; i += 2) //开始删边	
	{
		...
		while (isDeleted[make_pair(pairHeap.top().v1, pairHeap.top().v2)] == true) {pairHeap.pop(); cout << "pop that deleted" << endl;}
		...
		for (set<int>::iterator it = v1->connectV.begin(); it != v1->connectV.end(); it++)//遍历v1的连接点
		{cout << "find v111111's connect v" << endl;
			if ((*it) != v2->id)
			{
		...
		{cout << "find v222222's connect v " << endl;
			if ((*it) != v1->id )
			{
		...
		vertex[v1->id].deleted = true; //标记结点已经被删除
		vertex[v2->id].deleted = true;
	}
}
...
int main(int argc, char** argv)
{
	...
	{
		inputfile = "bunny.fine.obj";
		outputfile = "bunny.fine2.obj";
		ratio = 0.8;
	}
	else
	{
